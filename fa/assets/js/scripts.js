...
certy.nav.tooltip.show = function (current) {
	certy.nav.tooltip.timer = setTimeout(function () {
		...
		// Get tooltip height
		var height = (certy.nav.tooltip.el.outerHeight() - current.height())/2;

		// Positioning tooltip
		if (left - certy.nav.tooltip.el.outerWidth() > 0) {
			certy.nav.tooltip.el.addClass('arrow-left').css({
				"left": (left - certy.nav.tooltip.el.outerWidth()) + "px",
				"top": (top - height) + "px"
			});
		} else {
...
$(function () { // start: document ready
	...
	// Open Sidebar
	ace.sidebar.btn.on('click', function () {
		ace_open_sidebar();
	});
	 ...
	 */
	if ($('#map').length > 0 && $('#map iframe').length == 0) {
		initialiseGoogleMap();
	}
